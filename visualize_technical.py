...
def annotate_at_levels(patterns, vis_sup_zones, vis_res_zones,
                       sup_tls, res_tls, df):
    """
...
def compute_entry_signals(patterns: list, df: pd.DataFrame,
    ...
    for p in patterns:
        ...
        if is_candle:
            ...
            neckline = kp.get("B") or kp.get("neck", 0)
            rs_or_c  = min(kp.get("C", float("inf")),
                           kp.get("RS", float("inf")),
                           kp.get("A", float("inf")))
        ...
        risk_pct = (abs(entry_price - stop_loss) / entry_price * 100
                    if entry_price and stop_loss and entry_price > 0 else 0.0)

        # ── signal code ──
        if forming:
            signal = ("WATCH-LONG" if ptype == "bullish"
                      else "WATCH-SHORT" if ptype == "bearish"
                      else "WATCH")
        else:
            if ptype == "bullish":
...
def draw_patterns(ax, patterns: list, df: pd.DataFrame, price_range: float) -> None:
    ...
    for p in patterns:
        ...
        if "segments" in p:
            ...
            for seg in p["segments"]:
                ax.plot(seg["x"], seg["y"], color=color,
                        linewidth=lw, linestyle="--", alpha=alp, zorder=5)
        elif len(idxs) >= 2:
            if p["type"] == "bearish":
            ...
            ls = "--" if forming else ":"
            ax.plot(idxs, ys, color=color, linewidth=lw,
                    linestyle=ls, alpha=alp, zorder=5)
        ...
        if bar not in seen_bars or seen_bars[bar][0] > priority:
            sig = p.get("signal", "")
            sig_icon = ("^" if "LONG" in sig or sig in ("BUY", "BUY?")
                        else "v" if "SHORT" in sig or sig in ("SELL", "SELL?")
                        else "~")
            sig_str = f"{sig_icon} {sig}" if sig else ""
            if at_level:
                label = f"{p['short']}\n{at_level}\n{sig_str}" if sig_str else f"{p['short']}\n{at_level}"
            elif sig_str:
                label = f"{p['short']}\n{sig_str}"
            else:
                label = p["short"]
            seen_bars[bar] = (priority, label, color,
                              p["label_price"], p["label_above"], forming, at_level)
...
def find_fib_pivots(df: pd.DataFrame, p_highs: list, p_lows: list,
                    current_price: float):
    """
    ...
    lows_before_high = [p for p in recent_lows
                        if p["price"] < current_price * 0.999
                        and p["index"] < swing_high_p["index"]]
...
def find_fib_pivots_bearish(df: pd.DataFrame, p_highs: list, p_lows: list,
                            current_price: float):
    """
    ...
    highs_before_low = [p for p in recent_highs
                        if p["price"] > current_price * 1.001
                        and p["index"] < swing_low_p["index"]]
...
def draw_fib_levels(ax, swing_low: float, swing_high: float,
                    swing_low_idx: int, swing_high_idx: int,
                    n: int, current_price: float,
    ...
    if direction == "bullish":
        ...
        RETRACE_COLORS = ["#ef5350", "#ffd54f", "#ffb300", "#ff9800",
                          "#ff6d00", "#bf360c", "#69f0ae"]
        ls = "--"
        zone_color = "#ffd54f"
        prefix = "▲"
        ...
        for (ratio, label), color in zip(RETRACE_RATIOS, RETRACE_COLORS, strict=True):
            ...
            ax.text(n + 0.5, price, f"{prefix} {label}  {price:.2f}",
                    color=color, fontsize=5.5, va="center", alpha=0.72, zorder=7)

        # Upside extensions above swing_high
        for ratio, color, label in [(1.272, "#ce93d8", "1.272 T2"),
                                     (1.618, "#9c27b0", "1.618 T3")]:
            price = swing_low + ratio * move
            ax.axhline(price, color=color, linewidth=1.1,
                       linestyle=ls, alpha=0.85, zorder=2)
            ax.text(n + 0.5, price, f"{prefix} {label}  {price:.2f}",
                    color=color, fontsize=6.5, va="center", alpha=0.95,
        ...
        RETRACE_COLORS = ["#69f0ae", "#bf360c", "#ff6d00", "#ff9800",
                          "#ffb300", "#ffd54f", "#ef5350"]
        ls = ":"
        zone_color = "#ef5350"
        prefix = "▼"
        ...
        for ratio, color, label in [(1.272, "#ef9a9a", "1.272 T2"),
                                     (1.618, "#b71c1c", "1.618 T3")]:
            price = swing_high - ratio * move
            ax.axhline(price, color=color, linewidth=1.1,
                       linestyle=ls, alpha=0.85, zorder=2)
            ax.text(n + 0.5, price, f"{prefix} {label}  {price:.2f}",
...
def save_chart(ticker: str, period: str,
               df: pd.DataFrame, result: dict,
               patterns: list, p_highs: list, p_lows: list) -> str:
    """
    ...
    vis_sup_zones = select_zones(result.get("support_zones",    []), current_price, "support",    MAX_SUP_ZONES)
    vis_res_zones = select_zones(result.get("resistance_zones", []), current_price, "resistance", MAX_RES_ZONES)
    vis_lt_sup    = select_trendlines(result.get("long_term_support_trendlines",    []), current_price, "support")
    vis_lt_res    = select_trendlines(result.get("long_term_resistance_trendlines", []), current_price, "resistance")
    vis_st_sup    = select_trendlines(result.get("short_term_support_trendlines",   []), current_price, "support")
    vis_st_res    = select_trendlines(result.get("short_term_resistance_trendlines",[]), current_price, "resistance")
    ...
    if fib_bull:
        draw_fib_levels(ax, **fib_bull, n=n, current_price=current_price,
                        direction="bullish")
    ...
    if fib_bear:
        draw_fib_levels(ax, **fib_bear, n=n, current_price=current_price,
                        direction="bearish")
    ...
    ax.legend(handles=handles + zone_patches,
              ...
              labelcolor="#b2b5be", fontsize=8, framealpha=0.9, markerscale=0.9)
...
def main():
    ...
    annotate_at_levels(patterns, vis_sup_zones, vis_res_zones,
                       vis_lt_sup + vis_st_sup, vis_lt_res + vis_st_res, df)
