...
def _apply_rescreen_rules(
    ticker: str, holding: dict, fund_row: dict | None, yf_d: dict, price: float
) -> dict:
    """
...
def cmd_status(args):
    ...
    total_gain_pct = (
        (total_port_value - total_invested) / total_invested if total_invested > 0 else 0
    )
...
def parse_args():
    ...
    group.add_argument(
        "--scan", action="store_true", help="Screen DEFAULT_US for long-term candidates (default)"
    )
    group.add_argument("--add", type=str, metavar="TICKER", help="Add a tranche to a position")
    group.add_argument(
        "--rescreen",
        action="store_true",
    ...
    group.add_argument("--exit", type=str, metavar="TICKER", help="Manually close a position")
    group.add_argument(
        "--capital", type=float, metavar="N", help="Set total capital (default 100000)"
    )
    ...
    p.add_argument(
        "--reason", type=str, default="MANUAL", help="Exit reason tag for --exit (default: MANUAL)"
    )
    return p.parse_args()
