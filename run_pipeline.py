...
def ohlcv_for(price_df: pd.DataFrame, ticker: str,
    ...
    try:
        ...
        sub = pd.DataFrame({c: price_df[c][ticker] for c in cols
                            if c in price_df.columns.get_level_values(0)})
...
def get_technical_signal(ticker: str, df: pd.DataFrame,
    ...
    try:
        ...
        bullish = [p for p in patterns
                   ...
                   and p["entry_price"] > 0 and p["stop_loss"] > 0]
        if not bullish:
            return None
...
def run_pipeline():
    ...
    for phase_idx, phase in enumerate(PHASES):
        ...
        for stock in selected:
            ...
            logger.info(f"  {stock['ticker']}: {trade['exit_reason']:4s}  "
    ...
    for rec in portfolio_history:
        ...
        rec["portfolio_drawdown"]  = (rec["portfolio_value"]  - max_port)  / max_port  if max_port  else 0.0
        rec["benchmark_drawdown"]  = (rec["benchmark_value"]  - max_bench) / max_bench if max_bench else 0.0
    ...
    with open("reports/pipeline_summary.json", "w") as f:
        json.dump({"phases": phase_summaries, "trades": len(trade_logs),
                   "final_capital": round(final_port, 2)}, f, indent=2)
...
def run_lt_backtest():
    ...
    for phase_idx, phase in enumerate(PHASES):
            ...
            for ticker in active:
                ...
                if reason:
                    try:
                        ...
                        logger.info(f"  {ticker}: {reason} @ {exit_day.date()}  "
        ...
        for ticker, h in holdings.items():
            ...
            trade_logs.append(trade)
            logger.info(f"  {ticker}: {exit_reason:20s}  "
        ...
        last_nav     = portfolio_history[-1]["portfolio_value"] if portfolio_history else current_capital
