...
class MarketStructureEngine:
    ...
    def analyze_structure(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        long_term_support = self._detect_trendlines(
            df, p_lows, "support", current_price, term="long"
        )
        long_term_resistance = self._detect_trendlines(
            df, p_highs, "resistance", current_price, term="long"
        )
        short_term_support = self._detect_trendlines(
            df, p_lows, "support", current_price, term="short"
        )
        short_term_resistance = self._detect_trendlines(
            df, p_highs, "resistance", current_price, term="short"
        )

        # 4. Classify current context using long-term trendlines
        context = self._classify_context(
            current_price, support_zones, resistance_zones, long_term_support, long_term_resistance
        )
    ...
    def _build_horizontal_zones(
        self, df: pd.DataFrame, pivots: list[dict[str, Any]], level_type: str, current_price: float
    ) -> list[dict[str, Any]]:
        """
        ...
        clustering = AgglomerativeClustering(
            distance_threshold=distance_threshold, n_clusters=None, linkage="complete"
        )
        ...
        levels_to_score = [
            {"price": float(np.median(cluster_prices))} for cluster_prices in clusters_dict.values()
        ]
        ...
        for idx, (level_dict, score_tuple) in enumerate(
            zip(levels_to_score, scorer.scores, strict=False)
        ):
    ...
    def _detect_trendlines(
        ...
        if len(local_pivots) < 2:
            ...
            if line_type == "support":
                local_pivots = (
                    auto_lows
                    if term == "long"
                    else [p for p in auto_lows if p["index"] >= max(0, n_days - 120)]
                )
            else:
                local_pivots = (
                    auto_highs
                    if term == "long"
                    else [p for p in auto_highs if p["index"] >= max(0, n_days - 120)]
                )
        ...
        def snap_anchor(idx: int):
            ...
            sub = df["Low"].iloc[lo:hi] if line_type == "support" else df["High"].iloc[lo:hi]
            if line_type == "support":
        ...
        for i in range(len(local_pivots)):
            for j in range(i + 1, len(local_pivots)):
                ...
                strength_score = (
                    (n_touches * 20.0) + (recency_factor * 25.0) + (avg_reaction * 100.0)
                )
                if term == "long":
                    strength_score += 15.0
        ...
        for cand in candidates:
            ...
            for accepted in filtered:
                ...
                val_close = (
                    abs(cand["current_value"] - accepted["current_value"])
                    / (accepted["current_value"] + 1e-8)
                    <= 0.02
                )
    ...
    def _classify_context(
        ...
        for r in resistances:
            if r["center_price"] >= current_price:
                if (
                    ...
                    or r["center_price"] < closest_resistance["center_price"]
                ):
        ...
        if (
            closest_support
            and abs(current_price - closest_support["center_price"])
            / closest_support["center_price"]
            <= 0.015
        ):
        ...
        if (
            closest_resistance
            and abs(closest_resistance["center_price"] - current_price) / current_price <= 0.015
        ):
