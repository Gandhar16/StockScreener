...
def generate_decision(
    ...
    metrics.get("gross_margin_ttm", 0.0)

    if bq >= 70.0:
        strengths.append(
            f"Exceptional business quality: ROIC/ROE of {max(roic, roe)*100:.1f}% and strong operating margins of {op_margin*100:.1f}%."
        )
    elif bq < 45.0:
        weaknesses.append(
            f"Subpar profitability: Low ROIC/ROE of {max(roic, roe)*100:.1f}% and thin margins."
        )
    ...
    if val >= 70.0:
        strengths.append(
            f"Highly attractive valuation: Trading at a reasonable P/E of {pe:.1f} (Forward P/E of {forward_pe:.1f})."
        )
    elif val < 40.0:
        weaknesses.append(
            f"Premium valuation: Stock is priced for perfection with P/E of {pe:.1f} (Forward P/E of {forward_pe:.1f})."
        )

    # Financial risk indicators
    de = metrics.get("debt_to_equity_ttm", 0.0)
    metrics.get("current_ratio_ttm", 0.0)
    if risk >= 75.0:
        strengths.append("Robust balance sheet: Low debt levels and strong short-term liquidity.")
    elif risk < 50.0:
        weaknesses.append(
            f"Leverage concerns: Debt-to-Equity is elevated at {de:.2f} and liquidity margins are thin."
        )
    ...
    if growth >= 70.0:
        strengths.append(
            f"Strong, durable growth profile: 3-year revenue growth average is {rev_g*100:.1f}% and EPS growth is {eps_g*100:.1f}%."
        )
    elif growth < 45.0:
        weaknesses.append(
            "Stagnant growth profile: Revenue and earnings growth rates are low or negative."
        )
    ...
    if red_flags:
        ...
        if risk < 55.0:
            risks.append(
                "Debt levels are slightly elevated and could present risk in an economic downturn."
            )
        ...
        if "utility" in sector.lower() or "energy" in sector.lower():
            risks.append(
                "Regulated returns or commodity prices represent major macro sensitivities."
            )
        if not risks:
            risks.append(
                "Standard market risks and potential execution delays in new product initiatives."
            )
