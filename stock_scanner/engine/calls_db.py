...
def upsert_call(call: Dict[str, Any], call_type: str) -> int:
    ...
    with _connect() as conn:
        existing = conn.execute(
            "SELECT id FROM calls WHERE ticker=? AND call_type=? "
            "AND status IN ('BUY','HOLD')",
            (ticker, call_type)
        ).fetchone()
        ...
        if existing:
            ...
            conn.execute("""
                ...
                WHERE id=?
            """, (
                call.get("name"), call.get("conviction"),
                call.get("signal"), call.get("pattern"),
                entry_price,                          # only fills in if NULL
                call.get("stop_loss"),
                call.get("t1"), call.get("t2"), call.get("t3"),
                call.get("fair_value"), call.get("upside_pct"),
                call.get("fund_score"),
                call.get("sentiment_label"), call.get("sentiment_score"),
                now, raw, call_id,
            ))
        else:
            cur = conn.execute("""
            ...
            """, (
                ticker, call.get("name"), call_type,
                call.get("conviction"), call.get("signal"), call.get("pattern"),
                call.get("date") or today, entry_price, call.get("stop_loss"),
                call.get("t1"), call.get("t2"), call.get("t3"),
                call.get("fair_value"), call.get("upside_pct"),
                call.get("fund_score"),
                call.get("sentiment_label"), call.get("sentiment_score"),
...
def get_active_calls() -> List[Dict]:
    ...
    with _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM calls WHERE status IN ('BUY','HOLD') "
...
def get_all_calls(limit: int = 200) -> List[Dict]:
    ...
    with _connect() as conn:
        rows = conn.execute(
            f"SELECT * FROM calls ORDER BY call_date DESC LIMIT {limit}"
        ).fetchall()
    return [dict(r) for r in rows]
...
def get_call_history(call_id: int) -> List[Dict]:
    ...
    with _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM call_updates WHERE call_id=? ORDER BY update_date DESC",
            (call_id,)
        ).fetchall()
    return [dict(r) for r in rows]
...
def update_call(call_id: int, current_price: float, status: str,
    ...
    with _connect() as conn:
        ...
        """, (
            current_price, status, pnl_pct, pnl_abs,
            notes, recommendation, now,
            sentiment_label, conviction,
        ...
        conn.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?)
        """, (call_id, now, current_price, status, conviction,
              sentiment_label, pnl_pct, notes))
...
def close_call(call_id: int, exit_price: float, notes: str = "Manually sold"):
    ...
    update_call(call_id, exit_price, "SELL", pnl_pct, pnl_abs,
                notes, recommendation="Position sold.", exit_price=exit_price)
...
def export_portfolio_json(out_path: str = "dashboard/portfolio.json",
    ...
    payload = {
        ...
        "summary": {
            ...
            "best_call":  {"ticker": best["ticker"],  "pnl_pct": round((best.get("pnl_pct") or 0)*100, 2)}  if best  else None,
            "worst_call": {"ticker": worst["ticker"], "pnl_pct": round((worst.get("pnl_pct") or 0)*100, 2)} if worst else None,
        },
