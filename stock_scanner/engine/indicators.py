...
def bollinger(close: pd.Series, period: int = 20, num_std: float = 2.0) -> dict[str, pd.Series]:
...
def ema_stack(close: pd.Series) -> dict:
    ...
    stacked_bull = (
        v20 is not None and v50 is not None and v200 is not None and px > v20 > v50 > v200
    )
    stacked_bear = (
        v20 is not None and v50 is not None and v200 is not None and px < v20 < v50 < v200
    )
...
def rsi_divergence(
    close: pd.Series, rsi_s: pd.Series, lookback: int = 60, order: int = 5
) -> str | None:
    """
...
def score_pattern(p: dict, indicators: dict) -> int:
    ...
    if at_lvl in ("@Sup Zone", "@Res Zone"):
        lvl_s = 15
    elif at_lvl in ("@Sup TL", "@Res TL"):
        lvl_s = 10
    else:
        lvl_s = 0
    ...
    if is_bull:
        if 30 <= rsi_v <= 50:
            rsi_s = 15  # recovering from oversold
        elif 50 < rsi_v <= 60:
            rsi_s = 10  # trending up, not extended
        elif rsi_v < 30:
            rsi_s = 12  # very oversold — bounce fuel
        elif 60 < rsi_v <= 70:
            rsi_s = 6
        else:
            rsi_s = 0  # overbought > 70
    else:
        if 50 <= rsi_v <= 70:
            rsi_s = 15  # rolling over from overbought
        elif 40 <= rsi_v < 50:
            rsi_s = 10  # trending down
        elif rsi_v > 70:
            rsi_s = 12  # very overbought — short fuel
        elif 30 <= rsi_v < 40:
            rsi_s = 6
        else:
            rsi_s = 0  # oversold < 30
    ...
    if is_bull:
        if bull_x:
            macd_s = 10  # fresh bullish cross
        elif macd_hi > 0:
            macd_s = 6  # bullish momentum
        else:
            macd_s = 0
    else:
        if bear_x:
            macd_s = 10  # fresh bearish cross
        elif macd_hi < 0:
            macd_s = 6  # bearish momentum
        else:
            macd_s = 0
...
def forming_confidence(p: dict, df: pd.DataFrame, indicators: dict) -> dict:
    ...
    if is_bull:
        if 35 <= rsi_v <= 55:
            adj += 7  # neutral — fuel left
        elif rsi_v < 35:
            adj += 5  # oversold — extra bounce fuel
        elif 55 < rsi_v <= 65:
            adj += 2
        else:
            adj -= 8  # overbought — little room
    else:
        if 45 <= rsi_v <= 65:
            adj += 7
        elif rsi_v > 65:
    ...
    # ── MACD alignment ───────────────────────────────────────────────────────
    if (is_bull and macd_hi > 0) or (not is_bull and macd_hi < 0):
        adj += 5
    else:
        adj -= 3
    ...
    if trigger and trigger > 0:
        ...
        if dist_pct < 0.02:
            adj += 8  # within 2% — imminent breakout
        elif dist_pct < 0.05:
            adj += 4  # within 5%
