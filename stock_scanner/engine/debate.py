    ...
    def _calculate_adjustment(self, arguments: List[Argument], original_score: float) -> float:
        ...
        for arg in arguments:
            ...
            if arg.category == "valuation":
                if "attractive" in arg.claim.lower() or "cheap" in arg.claim.lower() or "margin of safety" in arg.claim.lower():
                    direction = 1
                elif "expensive" in arg.claim.lower() or "too high" in arg.claim.lower():
                    direction = -1
            elif arg.category == "quality":
                if "excellent" in arg.claim.lower() or "strong" in arg.claim.lower() or "compounding" in arg.claim.lower():
                    direction = 1
                elif "poor" in arg.claim.lower() or "below" in arg.claim.lower() or "destroy" in arg.claim.lower():
                    direction = -1
                ...
                if "red flag" in arg.claim.lower() or "risk" in arg.claim.lower() or "excessive" in arg.claim.lower() or "distress" in arg.claim.lower():
                    direction = -1
                elif "strong" in arg.claim.lower() or "improving" in arg.claim.lower() or "healthy" in arg.claim.lower():
                    direction = 1
            elif arg.category == "technical":
                if "bull" in arg.claim.lower() or "support" in arg.claim.lower() or "intact" in arg.claim.lower():
                    direction = 1
    ...
    def _determine_consensus(self, arguments: List[Argument], adjusted_score: float) -> str:
        ...
        for arg in arguments:
            ...
            if arg.agent in [AgentPersona.VALUE, AgentPersona.GROWTH, AgentPersona.QUALITY]:
                ...
                elif any(word in arg.claim.lower() for word in ["expensive", "too high", "poor", "risk", "slow", "destroy", "red flag", "distress"]):
                    bearish += arg.confidence
            elif arg.agent == AgentPersona.TECHNICAL:
                if any(word in arg.claim.lower() for word in ["bull", "support", "intact", "breakout"]):
                    bullish += arg.confidence
                elif any(word in arg.claim.lower() for word in ["bear", "resistance", "deteriorating", "avoid"]):
                    bearish += arg.confidence
            elif arg.agent == AgentPersona.RISK:
                if any(word in arg.claim.lower() for word in ["red flag", "risk", "high", "excessive", "distress", "unstable"]):
