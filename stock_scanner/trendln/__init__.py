...
def datefmt(xdate, cal=None):
    ...
    def mydate(x, pos):
        ...
        val = int(x + 0.5)
        if val < 0:
            return (xdate[0].to_pydatetime() - CustomBusinessDay(-val, calendar=cal)).strftime(
                "%Y-%m-%d"
            )
        elif val >= len(xdate):
            return (
                xdate[-1].to_pydatetime() + CustomBusinessDay(val - len(xdate) + 1, calendar=cal)
            ).strftime("%Y-%m-%d")
        else:
...
def plot_sup_res_learn(curdir, hist):
    ...
    if not os.path.isdir(os.path.join(curdir, "data")):
        os.mkdir(os.path.join(curdir, "data"))  # image folder
    ...
    def fig_slopeint():
        ...
        plt.annotate(
            r"Standard slope-intercept line equation: $f(x)=y=mx+b$" + "\n"
            ...
            + "\n"
            + r"Distance between Points: d=$\sqrt{(x_1-x_0)^2+(y_1-y_0)^2}$",
            (0, 0),
        )
        plt.subplot(spec[0, 0])
        m = (hist.Close.iloc[-3] - hist.Close.iloc[-1]) / -2
        b1, _b2 = hist.Close.iloc[-1] - m * 2, hist.Close.iloc[-3] - m * 0
        ...
        plt.plot(
            [len(hist.Close) - 3, len(hist.Close) - 1],
            [hist.Close.iloc[-3], hist.Close.iloc[-1]],
            "g--",
        )
        ...
        )  # .transformed(plt.gcf().dpi_scale_trans.inverted()) #convert pixels to points
        ...
        mvisual = (
            ...
            * drawdim[1]
            / yaxhgt
            / (-2 * drawdim[0] / xaxwdt)
        )  # scale is 2:yaxhgt, could use this in computations, but must do dynamically with event handler since draw scale changes
        ...
        intcpt = ((hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - ax.get_ylim()[0]) * drawdim[
            1
        ] / yaxhgt - (-(drawdim[0] / 2) / mvisual)
        ann = plt.annotate(
            rf"$d=\sqrt{{({hist.Close.iloc[-3]}-{hist.Close.iloc[-1]})^2+({0}-{2})^2}}={round(dist, 2)}$",
        ...
        plt.annotate(
            rf"$b={hist.Close.iloc[-1]}-{round(m, 2)}*{2}={hist.Close.iloc[-3]}-{round(m, 2)}*{0}={b1}$",
            (len(hist.Close) - 3, hist.Close.iloc[-3]),
            (len(hist.Close) - 3, hist.Close.iloc[-3] - height * 0.1),
            arrowprops={"arrowstyle": "->"},
        )
        plt.plot(
            [len(hist.Close) - 2, len(hist.Close) - 2], [m * 1 + b1, hist.Close.iloc[-2]], "r--"
        )
        plt.annotate(
            (r"$d=$" + "\n" + r"$\left|{}*{}+{}-{}\right|$" + "\n" + r"$={}$").format(
                round(m, 2), 1, b1, hist.Close.iloc[-2], round(d, 2)
            ),
            (len(hist.Close) - 2, (m * 1 + b1 + hist.Close.iloc[-2]) / 2),
            (len(hist.Close) - 2 + 0.1, (m * 1 + b1 + hist.Close.iloc[-2]) / 2),
            va="center",
            color="red",
            arrowprops={"arrowstyle": "-[", "color": "red"},
        )
        plt.annotate(
            rf"$m=\frac{{{round(hist.Close.iloc[-3] - hist.Close.iloc[-1], 2)}}}{{{0 - 2}}}={round(m, 2)}$",
            (len(hist.Close) - 2, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            (
                len(hist.Close) - 2 + 0.2,
                (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - height * 0.1,
            ),
            color="black",
            arrowprops={"arrowstyle": "->"},
        )
        plt.plot(
            [len(hist.Close) - 3, len(hist.Close) - 1],
            [hist.Close.iloc[-3], hist.Close.iloc[-3]],
            "c--",
        )
        plt.annotate(
            rf"$\Delta x={0}-{2}={0 - 2}$",
            (len(hist.Close) - 2, hist.Close.iloc[-3]),
            (len(hist.Close) - 2, hist.Close.iloc[-3] + height * 0.10),
            color="cyan",
            ha="center",
            va="center",
        ...
        plt.plot(
            [len(hist.Close) - 1, len(hist.Close) - 1],
            [hist.Close.iloc[-3], hist.Close.iloc[-1]],
            "c--",
        )
        plt.annotate(
            rf"$\Delta y={hist.Close.iloc[-3]}-{hist.Close.iloc[-1]}={round(hist.Close.iloc[-3] - hist.Close.iloc[-1], 2)}$",
            (len(hist.Close) - 1, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            (len(hist.Close) - 2 + 0.5, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            color="cyan",
            ha="center",
            va="center",
            arrowprops={"arrowstyle": "-[", "color": "cyan"},
        ...
        plt.savefig(
            os.path.join(curdir, "data", "slopeint.svg"), format="svg"
        )  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(
            os.path.join(curdir, "data", "slopeint.png"), format="png"
        )  # , bbox_inches = extent, pad_inches = 0)
        # plt.show()
    ...
    def fig_linregrs():
        ...
        plt.annotate(
            ...
            + "  "
            + r"Standard Error of Intercept: $\sigma_b=\sigma_m\sqrt{\frac{\sum_{i=1}^nx_i^2}{n}}$",
            (0, 0),
        )
        plt.subplot(spec[0, 0])
        ...
        xbar, ybar = (
            (0 + 1 + 2) / 3,
            (hist.Close.iloc[-3] + hist.Close.iloc[-2] + hist.Close.iloc[-1]) / 3,
        )
        height = hist.Close.iloc[-3:].max() - hist.Close.iloc[-3:].min()
        plt.hlines(ybar, len(hist.Close) - 3, len(hist.Close) - 1, colors="r", linestyles="--")
        plt.annotate(
            rf"$\bar{{x}}=\frac{{{0}+{1}+{2}}}{{{3}}}={1}$",
            (
                xbar + len(hist.Close) - 3,
                (hist.Close.iloc[-3:].min() + hist.Close.iloc[-3:].max()) / 2,
            ),
            (xbar + len(hist.Close) - 3, hist.Close.iloc[-3:].min()),
            color="red",
            va="center",
            arrowprops={"arrowstyle": "->", "color": "red"},
        )
        plt.vlines(
            xbar + len(hist.Close) - 3,
            hist.Close.iloc[-3:].min(),
            hist.Close.iloc[-3:].max(),
            colors="r",
            linestyles="--",
        )
        plt.annotate(
            rf"$\bar{{y}}=\frac{{{hist.Close.iloc[-3]}+{hist.Close.iloc[-2]}+{hist.Close.iloc[-1]}}}{{{3}}}={round(ybar, 2)}$",
            (len(hist.Close) - 2, ybar),
            (len(hist.Close) - 1, ybar - height * 0.1),
            color="red",
            va="top",
            ha="right",
            arrowprops={"arrowstyle": "->", "color": "red"},
        ...
        m = (
            (0 - xbar) * (hist.Close.iloc[-3] - ybar)
            + (1 - xbar) * (hist.Close.iloc[-2] - ybar)
            + (2 - xbar) * (hist.Close.iloc[-1] - ybar)
        ) / (np.square(0 - xbar) + np.square(1 - xbar) + np.square(2 - xbar))
        b = ybar - m * xbar
        SSR = (
            np.square(hist.Close.iloc[-3] - (m * 0 + b))
            + np.square(hist.Close.iloc[-2] - (m * 1 + b))
            + np.square(hist.Close.iloc[-1] - (m * 2 + b))
        )
        err1 = np.sqrt(
            SSR / ((3 - 2) * (np.square(0 - xbar) + np.square(1 - xbar) + np.square(2 - xbar)))
        )
        err2 = err1 * np.sqrt((np.square(0) + np.square(1) + np.square(2)) / 3)
        plt.annotate(
            rf"$\hat{{y}}_0={round(m, 2)}*{0}+{round(b, 2)}={round(m*0+b, 2)}$",
            (len(hist.Close) - 3, m * 0 + b),
            (len(hist.Close) - 3 + 0.1, m * 0 + b),
            va="top",
            arrowprops={"arrowstyle": "->"},
        )
        plt.annotate(
            rf"$\hat{{y}}_1={round(m, 2)}*{1}+{round(b, 2)}={round(m*1+b, 2)}$",
            (len(hist.Close) - 2, m * 1 + b),
            (len(hist.Close) - 2 + 0.15, m * 1 + b + height * 0.01),
            arrowprops={"arrowstyle": "->"},
        )
        plt.annotate(
            rf"$\hat{{y}}_2={round(m, 2)}*{2}+{round(b, 2)}={round(m*2+b, 2)}$",
            (len(hist.Close) - 1, m * 2 + b),
            (len(hist.Close) - 1 - 0.1, m * 2 + b + height * 0.1),
            ha="right",
            arrowprops={"arrowstyle": "->"},
        )
        plt.plot(
            [len(hist.Close) - 3, len(hist.Close) - 3], [hist.Close.iloc[-3], ybar], color="green"
        )
        plt.plot(
            [len(hist.Close) - 2, len(hist.Close) - 2], [hist.Close.iloc[-2], ybar], color="green"
        )
        plt.plot(
            [len(hist.Close) - 1, len(hist.Close) - 1], [hist.Close.iloc[-1], ybar], color="green"
        )
        plt.annotate(
            rf"$y_0-\bar{{y}}={round(hist.Close.iloc[-3] - ybar, 2)}$",
            (len(hist.Close) - 3, (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1),
            (len(hist.Close) - 3 + 0.1, (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1),
            color="green",
            va="center",
            arrowprops={"arrowstyle": "-[", "color": "green"},
        )
        plt.annotate(
            rf"$y_1-\bar{{y}}={round(hist.Close.iloc[-2] - ybar, 2)}$",
            (len(hist.Close) - 2, (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1),
            (len(hist.Close) - 2 + 0.1, (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1),
            color="green",
            va="center",
            arrowprops={"arrowstyle": "-[", "color": "green"},
        )
        plt.annotate(
            rf"$y_2-\bar{{y}}={round(hist.Close.iloc[-1] - ybar, 2)}$",
            (len(hist.Close) - 1, (hist.Close.iloc[-1] + ybar) / 2),
            (len(hist.Close) - 1 - 0.1, (hist.Close.iloc[-1] + ybar) / 2),
            color="green",
            va="center",
            ha="right",
            arrowprops={"arrowstyle": "-[", "color": "green"},
        ...
        plt.plot(
            [len(hist.Close) - 3, len(hist.Close) - 3],
            [hist.Close.iloc[-3], m * 0 + b],
            color="cyan",
        )
        plt.plot(
            [len(hist.Close) - 2, len(hist.Close) - 2],
            [hist.Close.iloc[-2], m * 1 + b],
            color="cyan",
        )
        plt.plot(
            [len(hist.Close) - 1, len(hist.Close) - 1],
        ...
        plt.annotate(
            rf"$y_0-\hat{{y}}={round(hist.Close.iloc[-3] - (m*0+b), 2)}$",
            (len(hist.Close) - 3, (hist.Close.iloc[-3] + m * 0 + b) / 2),
            (len(hist.Close) - 3 + 0.1, (hist.Close.iloc[-3] + m * 0 + b) / 2),
            color="cyan",
            va="center",
            arrowprops={"arrowstyle": "-[", "color": "cyan"},
        )
        plt.annotate(
            rf"$y_1-\hat{{y}}={round(hist.Close.iloc[-2] - (m*1+b), 2)}$",
            (len(hist.Close) - 2, (hist.Close.iloc[-2] + m * 1 + b) / 2),
            (len(hist.Close) - 2 + 0.1, (hist.Close.iloc[-2] + m * 1 + b) / 2),
            color="cyan",
            va="center",
            arrowprops={"arrowstyle": "-[", "color": "cyan"},
        )
        plt.annotate(
            rf"$y_2-\hat{{y}}={round(hist.Close.iloc[-1] - (m*2+b), 2)}$",
            (len(hist.Close) - 1, (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05),
            (len(hist.Close) - 1 - 0.1, (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05),
            color="cyan",
            va="center",
            ha="right",
            arrowprops={"arrowstyle": "-[", "color": "cyan"},
        ...
        plt.annotate(
            (
                ...
                0,
                round(xbar, 2),
                round(hist.Close.iloc[-3] - ybar, 2),
                1,
                round(xbar, 2),
                round(hist.Close.iloc[-2] - ybar, 2),
                2,
                round(xbar, 2),
                round(hist.Close.iloc[-1] - ybar, 2),
                0,
                round(xbar, 2),
                1,
                round(xbar, 2),
                ...
                round(xbar, 2),
                round(m, 2),
                round(hist.Close.iloc[-3] - (m * 0 + b), 2),
                round(hist.Close.iloc[-2] - (m * 1 + b), 2),
                round(hist.Close.iloc[-1] - (m * 2 + b), 2),
                round(SSR, 2),
                round(SSR, 2),
                3,
                0,
            ...
            (len(hist.Close) - 2, m * 1 + b),
            (len(hist.Close) - 1, hist.Close.iloc[-3:].min()),
            color="blue",
            va="bottom",
            ha="right",
        ...
        plt.annotate(
            rf"$b={round(ybar, 2)}-{round(m, 2)}*{xbar}={round(b, 2)}$",
            (len(hist.Close) - 3, b),
            (len(hist.Close) - 3 + 0.1, b),
            color="blue",
            ha="left",
            arrowprops={"arrowstyle": "->", "color": "blue"},
        )
        ...
        # extent = plt.gcf().get_window_extent(renderer=plt.gcf().canvas.get_renderer()).transformed(plt.gcf().dpi_scale_trans.inverted())
        plt.savefig(
            os.path.join(curdir, "data", "linregrs.svg"), format="svg"
        )  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(
            os.path.join(curdir, "data", "linregrs.png"), format="png"
        )  # , bbox_inches = extent, pad_inches = 0)
    ...
    def fig_hough():
        ...
        plt.gca().get_yaxis().set_visible(False)
        plt.annotate(
            r"Slope of Perpendicular Line: $m_p=-\frac{1}{m}, mm_p=-1$"
            + "\n"
        ...
        plt.subplot(spec[0, 0])
        plt.plot(
            [len(hist.Close) - 10, len(hist.Close) - 1],
            [hist.Close.iloc[-10], hist.Close.iloc[-1]],
            "ro",
        )
        plt.plot(
            [len(hist.Close) - 10, len(hist.Close) - 1],
        ...
        mn, mx = (
            min(hist.Close.iloc[-10], hist.Close.iloc[-1]),
            max(hist.Close.iloc[-10], hist.Close.iloc[-1]),
        )
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 1], [mn, mx], "b--")
        plt.annotate(
            rf"Diagonal length=$\sqrt{{{9}^2+{round(mx-mn, 2)}^2}}={round(np.sqrt(np.square(9)+np.square(mx-mn)), 2)}$",
            (len(hist.Close) - 1, mx),
            (len(hist.Close) - 1 - 1, mx),
            ha="right",
            va="top",
            color="blue",
            arrowprops={"arrowstyle": "->", "color": "blue"},
        ...
        plt.annotate(
            rf"$y={round(m, 2)}x+{round(b, 2)}$",
            (len(hist.Close) - 5.5, (mn + mx) / 2),
            (len(hist.Close) - 5.5, mn + (mx - mn) * 0.7),
            arrowprops={"arrowstyle": "->"},
        )
        ...
        plt.annotate(
            "",
            (len(hist.Close) - 10, mn),
            (len(hist.Close) - 10 + x0, mn + -x0 / m + bperp),
            arrowprops={"arrowstyle": "<|-", "color": "red"},
        )
        plt.gca().add_patch(
            mpatches.Wedge(
        ...
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10, mn), 3, 0, angle * 180 / np.pi, fill=False)
        )
        plt.annotate(
            rf"$\theta={round(angle * 180/np.pi, 2)}^\circ$", (len(hist.Close) - 6.75, mn + 0.1)
        )
        plt.gca().add_patch(
            mpatches.Wedge(
                (len(hist.Close) - 10, mx), 3, 270, 270 + angle * 180 / np.pi, fill=False
            )
        )
        ...
        plt.annotate(
            (
                ...
                round(m, 2),
                0,
                round(angle * 180 / np.pi, 2),
                round(hist.Close.iloc[-10] - mn, 2),
                round(angle * 180 / np.pi, 2),
                9,
                round(angle * 180 / np.pi, 2),
                hist.Close.iloc[-1] - mn,
                round(angle * 180 / np.pi, 2),
                round(0 * np.cos(angle) + (hist.Close.iloc[-10] - mn) * np.sin(angle), 2),
            ),
            (len(hist.Close) - 10 + x0 / 2, mn + (-x0 / m + bperp) / 2),
            (len(hist.Close) - 10 + x0 / 2, mn + (-x0 / m + bperp) / 2 + 0.9),
            ha="center",
            color="red",
            arrowprops={"arrowstyle": "->", "color": "red"},
        )
        ...
        plt.annotate(
            f"{9}",
            (len(hist.Close) - 5.5, mn),
            (len(hist.Close) - 5.5, mn + 0.5),
            ha="center",
            arrowprops={"arrowstyle": "->"},
        )
        plt.annotate(
            f"{round(mx-mn, 2)}",
            (len(hist.Close) - 10, (mn + mx) / 2),
            (len(hist.Close) - 10 + 0.5, (mn + mx) / 2),
            ha="left",
            arrowprops={"arrowstyle": "->"},
        )
        ...
        plt.savefig(
            os.path.join(curdir, "data", "hough.svg"), format="svg"
        )  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(
            os.path.join(curdir, "data", "hough.png"), format="png"
        )  # , bbox_inches = extent, pad_inches = 0)
    ...
    def fig_minima():
        ...
        minimaIdxs, _pmin, _mintrend, _minwindows = mins
        maximaIdxs, _pmax, _maxtrend, _maxwindows = maxs
        plt.clf()
        plt.gcf().set_size_inches(1024 / plt.gcf().dpi, 768 / plt.gcf().dpi)  # plt.gcf().dpi=100
        plt.subplot(111)
        ...
        for x in range(len(h)):
            ...
            plt.gca().add_artist(
                mpatches.FancyBboxPatch(
                    (min(ext.x0, next.x0) + 0.1, ext.y0),
                    width=max(ext.width, next.width) - 0.2,
                    height=ext.height + 3,
                    boxstyle="round",
                    fc="gray",
                    alpha=0.3,
            ...
            plt.gca().annotate(
                "", (x, h.Close.iloc[x]), (x, ext.y1 + 3), arrowprops={"arrowstyle": "-|>"}
            )
        plt.ylim(
            h.Close.min() - (h.Close.max() - h.Close.min()) / 2.8,
    ...
    def fig_reimann():
        ...
        _minimaIdxs, _pmin, _mintrend, _minwindows = mins
        _maximaIdxs, _pmax, maxtrend, _maxwindows = maxs
        plt.clf()
        ...
        plt.plot(
            range(base, trendline[0][-1] + 1),
            hist[-250:][base : trendline[0][-1] + 1].Close,
            "b-",
            label="Price",
        )
        plt.plot(
            (base, trendline[0][-1] + 1),
            (m * base + b, m * (trendline[0][-1] + 1) + b),
            "r-",
            label="Resistance",
        )
        plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(6))
        ...
        for x, y in enumerate(ser):
            plt.bar(
                x,
                (m * (x + base) + b) - y if isMin else y - (m * (x + base) + b),
                color="r"
                if (y < (m * (x + base) + b) if isMin else y > (m * (x + base) + b))
                else "gray",
            )

        plt.annotate(
            rf"S={round(S, 2)}, $\frac{{{round(S, 2)}}}{{{len(range(base, trendline[0][-1]+1))}}}$={round(area, 2)}$\frac{{\$}}{{day}}$"
            + "\n"
            + r"Reimann Sum where $\Delta x=x_i-x_{i-1}$,"
            + "\n"
            + r"$x_i^* \in [x_{i-1}, x_i]$: $S=\sum_{i=1}^n{f(x_i^*)\Delta x_i}$",
            (0, plt.gca().get_ylim()[0] + 5),
            va="bottom",
        )
        plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(6))
        # plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
    ...
    def fig_suppres():
        ...
        plt.savefig(
            os.path.join(curdir, "data", "suppreserr.svg"), format="svg", bbox_inches="tight"
        )
        plt.savefig(
            os.path.join(curdir, "data", "suppreserr.png"), format="png", bbox_inches="tight"
        )
...
METHOD_NCUBED, METHOD_NSQUREDLOGN, METHOD_HOUGHPOINTS, METHOD_HOUGHLINES, METHOD_PROBHOUGH = (
    0,
    1,
    2,
    3,
...
def check_num_alike(h):
    if (type(h) is list and all(isinstance(x, (bool, int, float)) for x in h)) or (
        type(h) is np.ndarray and h.ndim == 1 and h.dtype.kind in "biuf"
    ):
        return True
    else:
        import pandas as pd

        return bool(type(h) is pd.Series and h.dtype.kind in "biuf")
...
def get_extrema(h, extmethod=METHOD_NUMDIFF, accuracy=2, include_edge=False):
    ...
    if type(accuracy) is not int or accuracy <= 0 or accuracy % 2 != 0:
    ...
    if (
        ...
        and (h[0] is not None or h[1] is not None)
    ):
        hmin, hmax = h[0], h[1]
        if (
            h[0] is not None and h[1] is not None and len(hmin) != len(hmax)
        ...
        raise ValueError(
            "h is not list, numpy ndarray or pandas Series of numeric values or a 2-tuple thereof"
        )
    if extmethod == METHOD_NAIVE:
        ...
        def get_minmax(h):
            ...
            def minFunc(x):
                return len(x) == 3 and x.iloc[0] > x.iloc[1] and x.iloc[2] > x.iloc[1]

            def maxFunc(x):
                return len(x) == 3 and x.iloc[0] < x.iloc[1] and x.iloc[2] < x.iloc[1]

            def numdiff_extrema(func):
                return np.flatnonzero(rollwin.aggregate(func)).tolist()
        ...
        def get_minmax(h):
            ...
            def minFunc(x):
                return x.iloc[0] > x.iloc[1] and x.iloc[2] > x.iloc[1]
        ...
        def get_minmax(h):
            ...
            def numdiff_extrema(func):
                return [
                    ...
                    and (
                        mom[x]
                        == 0  # either slope is 0, or it crosses from positive to negative with the closer to 0 of the two chosen or prior if a tie
                        or (
                            (
                                x != len(mom) - 1
                                and (
                                    ...
                                    or (mom[x] < 0 and mom[x + 1] > 0 and h[x] <= h[x + 1])
                                )
                            )  # -mom[x] >= mom[x+1]) or
                            or (
                                x != 0
                                and (
                                    (
                                        mom[x - 1] > 0 and mom[x] < 0 and h[x - 1] < h[x]
                                    )  # mom[x-1] < -mom[x] or
                                    or (mom[x - 1] < 0 and mom[x] > 0 and h[x - 1] > h[x])
                                )
                            )
                        )
                    )
    ...
    if hmin is None and hmax is None:
        ...
        return edge_check(numdiff_extrema(minFunc), h, True), edge_check(
            numdiff_extrema(maxFunc), h, False
        )
    if hmin is not None:
    ...
    if hmax is not None:
        ...
        if hmin is None:
            return edge_check(maxf[2](maxf[1]), hmax, False)
    return edge_check(minf[2](minf[0]), hmin, True), edge_check(maxf[2](maxf[1]), hmax, False)
...
def calc_support_resistance(
    ...
    if (
        ...
        if (
            h[0] is not None and h[1] is not None and len(h[0]) != len(h[1])
        ):  # not strict requirement, but contextually ideal
            raise ValueError("h does not have a equal length minima and maxima data")
    ...
    def get_bestfit(pts):
        xbar, ybar = [sum(x) / len(x) for x in zip(*pts, strict=False)]

        def subcalc(x, y):
            tx, ty = x - xbar, y - ybar
            return tx * ty, tx * tx, x * x

        (xy, xs, xx) = [sum(q) for q in zip(*[subcalc(x, y) for x, y in pts], strict=False)]
        m = xy / xs
        b = ybar - m * xbar
    ...
    def get_trend(Idxs, h, fltpct, min_h, max_h):
        ...
        for x in range(
            len(Idxs)
        ):  # unfortunately an O(n(n-1)(n-2))=O((n^2-n)(n-2))=O(n^3-3n^2-2n)~=O(n^3) algorithm but meets the strict definition of a trendline
                ...
                for z in range(y + 1, len(Idxs)):
                    ...
                    trend.append(
                        (
                            ...
                            get_bestfit3(
                                Idxs[x], h[Idxs[x]], Idxs[y], h[Idxs[y]], Idxs[z], h[Idxs[z]]
                            ),
                        )
                    )
    ...
    def hough(Idxs, h, fltpct, min_h, max_h):
        ...
        for pts, angle, dist in zip(
            *hough_line_peaks(hl, theta, d, threshold=2), strict=False
        ):  # > threshold
            y0, y1 = (dist - origin * np.cos(angle)) / np.sin(angle)
            y0, y1 = y0 / scl + m, y1 / scl + m
    ...
    def prob_hough(Idxs, h, fltpct, min_h, max_h):
        ...
        lines = []
        for _x in range(hough_prob_iter):
            lines.extend(
                probabilistic_hough_line(
                    ...
                    line_length=0,
                    line_gap=int(
                        np.ceil(np.sqrt(np.square(image.shape[0]) + np.square(image.shape[1])))
                    ),
                )
            )
        l = []
    ...
    def window_results(trends, isMin, h):
        ...
        return [dosort([fitarea(pts) for pts in x]) for x in windows]
    ...
    def overall_line(idxs, vals):
        if len(idxs) <= 1:
            pm, _zme = [np.nan, np.nan], [np.nan]
        else:
            p, r = np.polynomial.polynomial.Polynomial.fit(
                idxs, vals, 1, full=True
            )  # more numerically stable
            pm, _zme = list(reversed(p.convert().coef)), r[0]
            if len(pm) == 1:
                pm.insert(0, 0.0)
        return pm
    ...
    def calc_all(idxs, h, isMin):
        ...
        mtrend = [
            (pts, (res[0], res[1], res[2], res[3], res[4], measure_area((pts, res), isMin, h)))
    ...
    if hmin is None and hmax is None:
        ...
        if hmin is not None:
            pmin, mintrend, minwindows = calc_all(
                extremaIdxs if hmax is None else extremaIdxs[0], hmin, True
            )
            if hmax is None:
                return (extremaIdxs, pmin, mintrend, minwindows)
        if hmax is not None:
            pmax, maxtrend, maxwindows = calc_all(
                extremaIdxs if hmin is None else extremaIdxs[1], hmax, False
            )
            if hmin is None:
                return (extremaIdxs, pmax, maxtrend, maxwindows)
    return (extremaIdxs[0], pmin, mintrend, minwindows), (
        extremaIdxs[1],
        pmax,
        maxtrend,
        maxwindows,
...
def get_levels(calc_result, x, price, n=3):
    ...
    if not (
        ...
        and isinstance(calc_result[0], tuple)
        and len(calc_result[0]) == 4
        and isinstance(calc_result[1], tuple)
        and len(calc_result[1]) == 4
    ):
        raise ValueError("calc_result must be the full 2-tuple output of calc_support_resistance")
    ...
    supports = sorted([t for t in all_levels if t[0] <= price], key=lambda t: price - t[0])[:n]
    resistances = sorted([t for t in all_levels if t[0] > price], key=lambda t: t[0] - price)[:n]
    ...
    for r_level, _, _, _ in resistances:
        if nearest_support is None:
            ...
            rr_ratios.append(float("inf") if risk == 0 else (r_level - price) / risk)
...
def get_horizontal_levels(h, pctbound=0.05, extmethod=METHOD_NUMDIFF, accuracy=2, min_touches=2):
    ...
    *min_touches* pivots.
    ...
    if is_tuple:
        ...
        raw_max_idxs = (
            get_extrema((None, h_high), extmethod, accuracy) if h_high is not None else []
        )
        support_pairs = [(float(h_low[i]), i) for i in raw_min_idxs] if h_low is not None else []
        resistance_pairs = (
            [(float(h_high[i]), i) for i in raw_max_idxs] if h_high is not None else []
        )
    else:
    ...
    def _cluster(price_idx_pairs):
        ...
        for price, idx in ordered[1:]:
            ...
            if (mean == 0.0 and price == 0.0) or (
                mean != 0.0 and abs(price - mean) / mean <= pctbound
            ):
                grp_prices.append(price)
                grp_idxs.append(idx)
            else:
                clusters.append(
                    (sum(grp_prices) / len(grp_prices), len(grp_prices), sorted(grp_idxs))
                )
                grp_prices = [price]
                grp_idxs = [idx]
        clusters.append((sum(grp_prices) / len(grp_prices), len(grp_prices), sorted(grp_idxs)))
...
def plot_sup_res_date(
    ...
    sortError=False,
    accuracy=2,
    title="Prices with Support/Resistance Trend Lines",
    y_axis_label="Price",
    series_label=None,
    show_average=True,
    ax=None,
    ...
    return plot_support_resistance(
        hist,
        ticker.FuncFormatter(datefmt(idx)),
        numbest,
        fromwindows,
        pctbound,
        extmethod,
...
def plot_support_resistance(
    ...
    ret = calc_support_resistance(
        hist, extmethod, method, window, errpct, hough_scale, hough_prob_iter, sortError, accuracy
    )
    if ohlc is not None:
        if (
            not isinstance(ohlc, tuple)
            or len(ohlc) != 4
            or not all(check_num_alike(s) for s in ohlc)
        ):
            raise ValueError(
    ...
    if len(ret) == 2:
        ...
        if (
            type(hist) is tuple
            and len(hist) == 2
            and check_num_alike(hist[0])
            and check_num_alike(hist[1])
        ):
            len_h = len(hist[0])
            ...
            dispwin = [
                (hist[0], minwindows, "Support", "g--"),
                (hist[1], maxwindows, "Resistance", "r--"),
            ]
            disptrend = [
                (hist[0], mintrend, "Support", "g--"),
                (hist[1], maxtrend, "Resistance", "r--"),
            ]
            if ohlc is None:
            ...
            disp = [
                (hist, minimaIdxs, pmin, "yo", "Avg. Support", "y--"),
                (hist, maximaIdxs, pmax, "bo", "Avg. Resistance", "b--"),
            ]
            dispwin = [
                (hist, minwindows, "Support", "g--"),
                (hist, maxwindows, "Resistance", "r--"),
            ]
        ...
        min_h, max_h = (
            min(hist[1 if hist[0] is None else 0]),
            max(hist[1 if hist[0] is None else 0]),
        )
        disp = [
            (hist[1], maximaIdxs, pmax, "bo", "Avg. Resistance", "b--")
            if hist[0] is None
            else (hist[0], minimaIdxs, pmin, "yo", "Avg. Support", "y--")
        ]
        dispwin = [
            (hist[1], maxwindows, "Resistance", "r--")
            if hist[0] is None
            else (hist[0], minwindows, "Support", "g--")
        ]
        disptrend = [
            (hist[1], maxtrend, "Resistance", "r--")
            if hist[0] is None
            else (hist[0], mintrend, "Support", "g--")
        ]
        if ohlc is None:
            ax.plot(
                range(len_h),
                hist[1 if hist[0] is None else 0],
                "k--",
        ...
        for ln in trend[:numbest]:
            if extend_to_end:
                ...
                while maxx < len_h:
                    ...
                    if (
                        (h[maxx] > ypred and h[maxx - 1] < ypred)
                        or (h[maxx] < ypred and h[maxx - 1] > ypred)
                        or ypred > max_h + (max_h - min_h) * pctbound
                        or ypred < min_h - (max_h - min_h) * pctbound
                    ):
                        break
                    maxx += 1
            ...
            if bFirst:
                ...
                ax.plot([ln[0][0], maxx], y_vals, clr)
        return bFirst
