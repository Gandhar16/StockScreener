...
class TouchScorer(BaseScorer):
    ...
    def closeFromExtreme(self, key, min_value, max_value):
        return abs(key - min_value) < (min_value * self.DIFF_PERC_FROM_EXTREME / 100.0) or abs(
            key - max_value
        ) < (max_value * self.DIFF_PERC_FROM_EXTREME / 100)
    ...
    def _get_level_score(self, candles, high_low_marks, price):
        ...
        for i in range(len(candles)):
            ...
            if (
                ...
                last_cut_pos = i
                events.append(
                    PointEvent(PointEventType.CUT_BODY, candle["Datetime"], self.score_for_cut_body)
                )
            # If the wick of the candle cuts through the price, then deduct some score
                ...
                last_cut_pos = i
                events.append(
                    PointEvent(PointEventType.CUT_WICK, candle["Datetime"], self.score_for_cut_body)
                )
            # If the if is close the high of some candle and it was in an uptrend, then add some score to this
                ...
                # Else add S2. S2 > S1
                    ...
                    events.append(
                        PointEvent(
                            PointEventType.TOUCH_UP, candle["Datetime"], self.score_for_touch_normal
                        )
                    )
