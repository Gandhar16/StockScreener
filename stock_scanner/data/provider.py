...
class DataProvider:
    ...
    def fetch_and_filter_prices(self, tickers: list[str]) -> pd.DataFrame:
        ...
        try:
            ...
            df = yf.download(
                tickers, period="5d", progress=False, group_by="ticker", auto_adjust=True
            )
        ...
        for ticker in tickers:
            try:
                ...
                if (
                    last_price >= self.config.filters.min_price
                    and avg_volume >= self.config.filters.min_volume
                ):
                    records.append(
                        {"ticker": ticker, "last_price": last_price, "avg_volume": avg_volume}
                    )
                else:
                    logger.debug(
                        f"Ticker {ticker} filtered out: Price={last_price:.2f}, AvgVol={avg_volume:.0f}"
                    )
            except KeyError:
    ...
    def fetch_fundamental_data(self, ticker: str) -> dict:
        ...
        try:
            ...
            metrics = {
                ...
                "rd_intensity": info.get("researchAndDevelopment", 0)
                / max(info.get("totalRevenue", 1), 1)
                if info.get("researchAndDevelopment")
                else None,
                ...
                "fcf_to_net_income_ttm": info.get("freeCashflow", 0)
                / max(info.get("netIncomeToCommon", 1), 1)
                if info.get("freeCashflow") and info.get("netIncomeToCommon")
                else None,
                "dividend_yield": info.get("dividendYield"),
                "price_to_sales": info.get("priceToSalesTrailing12Months"),
                "price_to_fcf": info.get("marketCap", 0) / max(info.get("freeCashflow", 1), 1)
