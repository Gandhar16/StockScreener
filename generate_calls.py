...
DEFAULT_US = [
    # Mega-cap tech
    "AAPL", "MSFT", "GOOGL", "NVDA", "AVGO", "META", "AMZN",
    "ORCL", "ADBE", "CRM", "NOW", "MU",
    ...
    "WMT", "PG", "KO", "HD", "MCD", "SBUX", "NKE", "COST", "TGT",
    # Energy
    "XOM", "CVX",
    # Industrials
    "BA", "CAT", "GE",
...
DEFAULT_INDIA = [
    ...
    "WIPRO.NS", "LT.NS", "BAJFINANCE.NS", "AXISBANK.NS", "SBIN.NS",
]
...
def run_technicals(ticker: str, period: str = "1y",
    ...
    try:
        ...
        if bullish:
            best_bull = min(bullish,
                            key=lambda p: (BUY_RANK[p["signal"]],
        ...
        if bearish:
            best_bear = min(bearish,
                            key=lambda p: (SELL_RANK[p["signal"]],
                                           not p.get("vol_confirmed", False),
                                           -p["completed_bar"]))
...
def parse_args():
    ...
    p.add_argument("--telegram-smart-money", action="store_true",
    ...
    for ticker in tickers:
        ...
        if fund_row:
            ...
            if score >= MIN_FUND_SCORE and "Avoid" not in rating:
                ...
                thesis = []
                if strengths:  thesis += [f"+ {s}" for s in strengths]
                if weaknesses: thesis += [f"- {w}" for w in weaknesses]
                if risks_list: thesis += [f"! {r}" for r in risks_list]

                # Technical context for entry timing
                tech_context = struct.get("context", "")
                at_support   = any(p.get("at_level") and "Sup" in str(p.get("at_level",""))
                ...
                lt_call = {
                    ...
                    "thesis":       thesis,
                    "entry_note":   ("Good technical entry — at support" if at_support
                                     else "Wait for pullback to support" if "Resistance" in tech_context
                    ...
                    "sentiment_note":    (f"Downgraded from {original_conv} — bearish news" if sent_flagged
                                         else "Supported by bullish news" if sent_boost else ""),
        ...
        if best_sig and best_sig.get("risk_reward", 0) >= MIN_RR:
            ...
            swing_call = {
                ...
                "sentiment_note":    (f"Downgraded from {sw_orig_conv} — bearish news" if sw_sent_flagged else ""),
                "news_count":        sent_count,
                "top_headlines":     sent_headlines,
            }
        ...
        if best_sell and best_sell.get("risk_reward", 0) >= MIN_RR:
            ...
            conv_s  = "HIGH CONVICTION" if (best_sell.get("vol_confirmed") and
                                             best_sell.get("signal") == "SELL" and
                                             best_sell.get("risk_reward", 0) >= 2.0) else \
                      "CONFIRMED" if best_sell.get("signal") in ("SELL", "SELL?") else "SETUP"
            sl_orig_conv = conv_s
            # For sell calls, bullish news is a headwind (downgrade); bearish news is a tailwind
            sl_sent_adj  = "BEARISH" if sent_label == "BULLISH" else \
                           "BULLISH" if sent_label == "BEARISH" else "NEUTRAL"
            ...
            sell_call = {
                ...
                "time_horizon":  "1–4 weeks" if not best_sell.get("forming") else "Wait for breakdown",
                # Sentiment (bullish news = headwind for a short)
                "sentiment_label":   sent_label,
                "sentiment_score":   sent_score,
                "sentiment_flagged": sl_sent_flagged,
                "sentiment_note":    (f"Downgraded from {sl_orig_conv} — bullish news is a headwind" if sl_sent_flagged else ""),
                "news_count":        sent_count,
                "top_headlines":     sent_headlines,
            }
