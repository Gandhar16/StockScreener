...
def send_message(
    token: str, chat_id: str, text: str, parse_mode: str = "HTML", disable_preview: bool = True
) -> bool:
    result = tg_request(
        token,
        "sendMessage",
...
def run_setup():
    ...
    override = input(
        f"\n  Use chat ID {chat_id}? (Enter to confirm, or type a different chat ID): "
    ).strip()
    if override:
        chat_id = override
...
def build_lt_section(calls: list[dict]) -> str:
    ...
    for c in calls:
        ...
        lines.append(
            f"   Price: <code>{price}</code>  FV: <code>{fv}</code>  Upside: <b>{upside}</b>"
        )
...
def build_swing_section(calls: list[dict]) -> str:
    ...
    for c in calls:
        ...
        lines.append(
            f"   Risk: {c.get('risk_pct',0):.1f}%  {sent} {esc(c.get('sentiment_label',''))}  ⏱ {esc(c.get('time_horizon',''))}"
        )
    return "\n".join(lines)
...
def build_smart_money_section(sm_data: dict, tickers: list[str]) -> str:
    ...
    for ticker in tickers:
        ...
        overall_icon = {"BULLISH": "🟢", "BEARISH": "🔴", "MIXED": "🟡", "NEUTRAL": "⚪"}.get(
            overall, "⚪"
        )
        ...
        lines.append(
            f"  {overall_icon} <b>{esc(ticker)}</b>  Ins:{ins_icon}{esc(ins_sig)}  HF:{hf_icon}{esc(hf_sig)}  News:{sentiment_icon(news_lbl)}"
        )
        ...
        if recent_ins:
            r = recent_ins[0]
            lines.append(
                f"    └ {esc(r['name'][:25])} ({esc(r['role'][:20])}) {esc(r['direction'])} {r.get('shares',0):,} sh"
            )
