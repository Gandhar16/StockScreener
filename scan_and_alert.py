        ...
        try:
            ...
            if not df.empty:
                for _, row in df.iterrows():
                    ...
                    passes = score >= MIN_FUND_SCORE and "Avoid" not in str(row.get("rating", ""))
                    universe[t] = {
                        "score": round(score, 2),
...
def update_universe(all_tickers: list[str], force: bool = False) -> dict:
    ...
    if force:
        ...
        tickers_to_screen = [
            t for t in all_tickers if t not in universe or ticker_needs_rescore(universe[t])
        ]
    ...
    try:
        ...
        if bullish:
            ...
            if best_bull.get("risk_reward", 0) < MIN_RR:
                ...
                best_bull.get("rs_pass") is False
                and best_bull.get("rs_mansfield", 0) is not None
                and best_bull.get("rs_mansfield", 0) <= -20
            ):
            ...
            best_bear.update(
                {
                    ...
                    "t2": t2_s,
                    "t3": t3_s,
                    "swing_low": fib_b["swing_low"],
                    "swing_high": fib_b["swing_high"],
                    "fib_levels": fib_b["fib_levels"],
                    "risk_reward": round((entry_s - t1_s) / risk_s, 2)
                    if risk_s > 0 and t1_s < entry_s
                    else 0,
                    "rr_t2": round((entry_s - t2_s) / risk_s, 2)
                    if risk_s > 0 and t2_s < entry_s
                    else 0,
                    "rr_t3": round((entry_s - t3_s) / risk_s, 2)
                    if risk_s > 0 and t3_s < entry_s
                    else 0,
                }
            )
...
def build_call_message(
    ...
    news_ok = sent_label == "BULLISH" if call_type == "BUY" else sent_label == "BEARISH"
    news_neu = sent_label == "NEUTRAL"
    ins_ok = ins_signal == "BUYING" if call_type == "BUY" else ins_signal == "SELLING"
    ins_neu = ins_signal in ("NEUTRAL", "MIXED", "NONE")
    hf_ok = hf_signal == "INCREASING" if call_type == "BUY" else hf_signal == "DECREASING"
    hf_neu = hf_signal in ("STABLE", "UNKNOWN")

    green = sum(
        [tech_ok, fund_ok, news_ok, ins_ok, hf_ok, bool(val_ok) if val_ok is not None else False]
    )
    ...
    if roic is not None and not (isinstance(roic, float) and math.isnan(roic)):
        roic_pct = roic * 100
        lines.append(
            f"ROIC     <code>{bar(min(roic_pct,100),100)}</code>  <b>{roic_pct:+.1f}%</b>  (3Y avg)"
        )
    if rev_g is not None and not (isinstance(rev_g, float) and math.isnan(rev_g)):
        rev_pct = rev_g * 100
        lines.append(
            f"Growth   <code>{bar(min(rev_pct,100),100)}</code>  <b>{rev_pct:+.1f}%</b>  (3Y avg)"
        )
    if op_m is not None and not (isinstance(op_m, float) and math.isnan(op_m)):
        om_pct = op_m * 100
        lines.append(
            f"Margin   <code>{bar(min(om_pct,100),100)}</code>  <b>{om_pct:+.1f}%</b>  (TTM)"
        )
    ...
    if forming and form_trigger:
        lines.append(
            f"  Trigger  <b>{price(form_trigger, cur)}</b>  ({'break above' if call_type=='BUY' else 'break below'})"
        )
        if form_move:
    ...
    if any(v is not None for v in (setup_sc, mtf_sc, rs_m)):
        ...
        if setup_sc is not None:
            lines.append(
                f"Setup    <code>{bar(setup_sc, 100, 8)}</code>  <b>{setup_sc}/100</b>  Grade <b>{esc(setup_gr or '—')}</b>"
            )
        if mtf_sc is not None:
            mtf_dot = "🟢" if mtf_ok else "🔴"
            lines.append(
                f"Weekly   {mtf_dot} {'aligned' if mtf_ok else 'AGAINST setup'}  ({mtf_sc}/100)"
            )
        if rs_m is not None:
            rs_dot = "🟢" if rs_m > 0 else ("🟡" if rs_m > -5 else "🔴")
            rank_txt = f"  ·  RS rank <b>{rs_rank}</b>/99" if rs_rank else ""
            lines.append(
                f"Rel Str  {rs_dot} Mansfield <b>{rs_m:+.1f}</b>  ({esc(rs_trend or '—')}){rank_txt}"
            )
        if rvol_v is not None:
            lines.append(f"RVOL     ×{rvol_v:.1f} today's volume vs 20d avg")
        if pos_shares:
            lines.append(
                f"Size     <b>{pos_shares}</b> sh  ·  risk {price(pos_risk, cur)}  ·  stop via {esc(stop_src or 'pattern')}"
            )
    ...
    # ══ SMART MONEY ═══════════════════════════════════════════════════════════
    ins_arrow = {"BUYING": "⬆️", "SELLING": "⬇️", "MIXED": "↔️", "NEUTRAL": "➖", "NONE": "➖"}.get(
        ins_signal, "➖"
    )
    hf_arrow = {"INCREASING": "⬆️", "DECREASING": "⬇️", "STABLE": "➖", "UNKNOWN": "➖"}.get(
        hf_signal, "➖"
    )
    ...
    for r in ins_recent[:2]:
        ...
        lines.append(
            f"  └ {d_icon} {esc(r['name'][:22])}  {r.get('shares',0):,} sh{val_s}  {r.get('date','')}"
        )
    ...
    for h in top_hf[:3]:
        ...
        lines.append(
            f"  └ {ci} {esc(h['name'][:28])}  {h.get('pct_held',0):.1f}%  ({chg:+.1f}% qtr)"
        )
...
def build_scan_summary(confirmed: list[dict], total_scanned: int, duration_s: float) -> str:
    ...
    if total == 0:
        ...
        lines.append(
            f"    🔥 {len(hc)} High Conviction  ·  ✅ {len(conf)} Confirmed  ·  👀 {len(setup)} Setup"
        )
        lines.append("")

        # Best call highlight
        best = next(
            (c for c in confirmed if c["conviction"] == "HIGH CONVICTION"),
            next((c for c in confirmed if c["conviction"] == "CONFIRMED"), confirmed[0]),
        )
        lines.append(
            f"⭐  <b>Top pick:  ${esc(best['ticker'])} {best['call_type']}</b>"
        ...
        def _summary_line(c):
            icon = (
                "🔥"
                if c["conviction"] == "HIGH CONVICTION"
                else "✅"
                if c["conviction"] == "CONFIRMED"
                else "👀"
            )
            rr = c.get("rr", 0) or 0
...
def scan_and_alert(
    ...
    for i, ticker in enumerate(passing, 1):
        ...
        if best_bull and best_bear:
            ...
            bear_raw = (
                "HIGH CONVICTION"
                if best_bear.get("vol_confirmed")
                and best_bear.get("signal") == "SELL"
                and best_bear.get("risk_reward", 0) >= 2.0
                else "CONFIRMED"
            ...
            if bb < br:
                ...
                bull_score = (
                    best_bull.get("vol_confirmed", False),
                    best_bull.get("risk_reward", 0),
                )
                bear_score = (
                    best_bear.get("vol_confirmed", False),
        ...
        for sig, call_type in [(best_bull, "BUY"), (best_bear, "SELL")]:
            ...
            if call_type == "BUY":
                ...
                conv, flagged = sentiment_adjust(
                    raw_conv, sent.get("sentiment_label", "NEUTRAL"), "SWING"
                )
            else:
                raw_conv = (
                    "HIGH CONVICTION"
                    if sig.get("vol_confirmed")
                    and sig.get("signal") == "SELL"
                    and sig.get("risk_reward", 0) >= 2.0
                    else "CONFIRMED"
                    if sig.get("signal") in ("SELL", "SELL?")
                    else "SETUP"
                )
                # For sells: bullish news = headwind
                sl_sent = (
                    "BEARISH"
                    if sent.get("sentiment_label") == "BULLISH"
                    else "BULLISH"
                    if sent.get("sentiment_label") == "BEARISH"
                    else "NEUTRAL"
                )
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(
            {"generated_at": datetime.now().isoformat(), "calls": save_payload},
            f,
            indent=2,
            default=str,
        )
...
def parse_args():
    ...
    p.add_argument(
        "--tickers", type=str, default=None, help="Comma-separated tickers (default: full S&P 500)"
    )
    ...
    p.add_argument(
        "--show-universe", action="store_true", help="Print the current strong universe and exit"
    )
    return p.parse_args()
